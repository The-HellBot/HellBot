...
async def callAPI(search_str):
    ...
    hburl = "https://graphql.anilist.co"
    response = requests.post(hburl, json={"query": query, "variables": variables})
...
async def anilist(event):
    ...
    json = (
        requests.post(hburl, json={"query": character_query, "variables": variables})
        .json()["data"]
        .get("Character", None)
    )
    if json:
        ...
        site_hburl = json.get("siteUrl")
        msg += shorten(description, site_hburl)
...
async def anilist(event):
    ...
    response = requests.post(
        hburl, json={"query": airing_query, "variables": variables}
    ).json()["data"]["Media"]
...
async def anilist(event):
    ...
    variables = {"search": search}
    json = (
        requests.post(hburl, json={"query": manga_query, "variables": variables})
        .json()["data"]
        .get("Media", None)
    )
    ms_g = ""
