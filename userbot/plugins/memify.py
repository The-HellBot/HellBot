...
base by: @HellBot_Official
created by: @Kraken_The_BadASS
Abeey change naa karo, maa chod denge tunhari🔥
"""
...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        await event.edit(
            "```Abhi maja aaega naa bhidu😁!Memifying this image! (」ﾟﾛﾟ)｣ ```"
        )
    ...
    async with borg.conversation("@MemeAutobot") as bot_conv:
        ...
        if "Okay..." in response.text:
            await event.edit(
                "```🤨 Ahh shit?! This is not an image! This will take some tym to convert to image fuk...🧐```"
            )
            thumb = None
