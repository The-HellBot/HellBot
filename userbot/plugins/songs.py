# Ported here by @Testpy12
# credit @Kraken_The_BadASS
# 🔨🛠HellBot🎼🎧🎧
import glob
...
async def _(event):
    ...
    if event.pattern_match.group(1):
        ...
        return
    event = await edit_or_reply(event, "`Finding Your Song!!`")
    try:
    ...
    if l:
        await event.edit("I think I got something,☺️")
    else:
...
async def _(event):
    ...
    if event.pattern_match.group(1):
        ...
        event = await edit_or_reply(event, "What I am Supposed to find")
        return
    event = await edit_or_reply(event, "Finding Your Videosong!!")
    ...
    if l:
        await event.edit("I think I found Something, 😉")
    else:
        await event.edit(f"Sorry..! i can't find anything with `{query}`")
        return
    thumbdark = glob.glob("./temp/*.jpg") + glob.glob("./temp/*.webp")
