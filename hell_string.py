...
if okvai == "69":
    ...
    if library == "1":
        ...
        with TelegramClient(StringSession(), APP_ID, API_HASH) as hellbot:
            ...
            hellbot.send_message("me", f"#HELLBOT #HELLBOT_SESSION #TELETHON\n\n`{hellbot.session.save()}`")
    elif library == "2":
        print("Pyrogram Session for HellBot")
        APP_ID = int(input("\nEnter APP ID here: "))
        API_HASH = input("\nEnter API HASH here: ")
        with Client(':memory:', api_id=APP_ID, api_hash=API_HASH) as hellbot:
            print("\nYour HellBot Session is sent in your Telegram Saved Messages.")
            hellbot.send_message("me", f"#HELLBOT #HELLBOT_SESSION #PYROGRAM\n\n`{hellbot.export_session_string()}`")
    else:
